"""Main solver port for ME_FLP_V7FixRhoscc.m (constant-reputation, rho'=rho)."""
...
def _parse_variant_and_alt(config: RunConfig) -> tuple[int, int, int]:
    ...
    if config.variant == "exo_rho":
        endog_rho = 0
    elif config.variant == "endog_rho":
        raise ValueError(
            "ME_FLP_V7FixRhoscc only supports VARIANT='exo_rho' (rho'=rho). "
            "Use ME_FLP_V6scc for endogenous rho."
        )
    else:
...
def run_model(config: RunConfig, options: SolverOptions | None = None) -> SolverResult:
    """Behavior-preserving translation of ME_FLP_V7FixRhoscc.m core flow."""
    ...
    # V7 (FixRho): rho' = rho fixed, so rhoprime / bilinear (rho', mu) interpolation
    # tables are not used in the inner loop. We still require d_mup == d_mu
    # (V7.m line 178: `d_mup=d_mu`) so the per-state Mhat slice can be indexed
    # directly along the mu axis.
    if d_mup.size != d_mu.size or not np.array_equal(d_mup, d_mu):
        raise ValueError(
            "ME_FLP_V7FixRhoscc requires d_mup == d_mu (rho'=rho, no mu interp). "
            f"Got n_mup={d_mup.size}, n_mu={d_mu.size}."
        )
        ...
        for in_loop in range(1, config.in_loop_max + 1):
            ...
            if (
                ...
                for st in range(config.n_squig, n_stend):
                    ...
                    # V7: direct 1-D slices (rho'=rho, d_mup==d_mu).
                    einfl_1d = (
                        config.bet * rho_st * Mhat1[i_squig, i_rhoi, :]
                        + config.bet * (1.0 - rho_st) * Mhat2[i_squig, i_rhoi, :]
                    )
                    Uhat_1d = Uhat[i_squig, i_rhoi, :]
                    einfl = np.broadcast_to(einfl_1d.reshape(1, -1), (config.n_delta, n_mup)).copy()
                    if optimizing == 1:
                    ...
                    ) + config.bet1 * (1.0 - q) * np.broadcast_to(
                        Uhat_1d.reshape(1, -1), (config.n_delta, n_mup)
                    )
            ...
            if (
                ...
                if target_st >= 0 and 0 <= target_i_mu < n_mu:
                    ...
                    einfl_1d_dbg = (
                        config.bet * rho_st_dbg * Mhat1[i_squig_dbg, i_rhoi_dbg, :]
                        + config.bet * (1.0 - rho_st_dbg) * Mhat2[i_squig_dbg, i_rhoi_dbg, :]
                    )
                    Uhat_1d_dbg = Uhat[i_squig_dbg, i_rhoi_dbg, :]
                    einfl_dbg = np.broadcast_to(
                        einfl_1d_dbg.reshape(1, -1), (config.n_delta, n_mup)
                    ).copy()
                    if optimizing == 1:
                    ...
                    urhs_dbg = ufcn_yl(
                        ...
                        Uhat_1d_dbg.reshape(1, -1), (config.n_delta, n_mup)
                    )
