...
def process_body_states_parallel(
    ...
    """V7 (FixRho) parallel per-state body worker.

    Mirrors the V6 Phase 5 worker but drops the (rho', mu) bilinear
    interpolation: V7 fixes rho' = rho, and d_mup == d_mu, so the per-state
    expected inflation reduces to a direct 1-D slice of Mhat plus broadcast
    across the delta axis::

        einfl_1d[j] = bet*rho*Mhat1[i_squig, i_rhost, j]
                    + bet*(1-rho)*Mhat2[i_squig, i_rhost, j]
        einfl[d, j] = einfl_1d[j]              # constant in delta
        Uhat_1d[j]  = Uhat[i_squig, i_rhost, j]

    The rest of the body (alpha_ic, a_ic, ufcn_yl, omega, F-flat ravel,
    fused argmax + tie detection, PTkeep / output writes) is identical to
    V6 Phase 5.

    The tie post-pass (`tie_record`, `aic_buf`, `einfl_buf`, `om_st_f_buf`,
    `urhs_f_buf`) follows the same design as V6 Phase 5: PTkeep is the
    correct numerical output (resolve_multiple_maxima is a no-op for
    PTkeep); only the diagnostic event count is reconstructed serially.
    """
    n_mu = d_mu.shape[0]
    one_m_q = 1.0 - q
    ...
    for st in prange(st_lo, st_hi):
        ...
        # V7 rho'=rho: direct 1-D slices of Mhat1, Mhat2, Uhat over mu axis.
        # einfl_1d (n_mup,), Uhat_1d (n_mup,) -- constant in delta when broadcast.
        einfl_1d = np.empty(n_mup, np.float64)
        Uhat_1d = np.empty(n_mup, np.float64)
        for j in range(n_mup):
            einfl_1d[j] = bet * rho_st * Mhat1[i_squig, i_rhoi, j] + bet * (1.0 - rho_st) * Mhat2[i_squig, i_rhoi, j]
            Uhat_1d[j] = Uhat[i_squig, i_rhoi, j]

        # einfl broadcast (constant in delta) and EINFL store
        einfl = np.empty((n_delta, n_mup), np.float64)
        for d in range(n_delta):
            for j in range(n_mup):
                einfl[d, j] = einfl_1d[j]
                EINFL[d, j, st] = einfl_1d[j]
...
def warmup() -> None:
    ...
    # Warm up the V7 parallel state worker with a 1-state run.
    n_squig = 1
    n_rho = 2
    n_mu_w = 2
    n_delta_w = 2
