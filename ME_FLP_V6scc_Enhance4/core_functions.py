...
from .jit_kernels import ii_fhat_factored_kernel
...
def ii_fhat_factored(
    ...
    fdens_arr = np.ascontiguousarray(np.asarray(fdens, dtype=np.float64).reshape(-1))
    ...
    rho_idx_c = np.ascontiguousarray(np.asarray(rho_idx, dtype=np.int64))
    rho_w_c = np.ascontiguousarray(np.asarray(rho_w, dtype=np.float64))
    mu_idx_c = np.ascontiguousarray(np.asarray(mu_idx, dtype=np.int64))
    mu_w_c = np.ascontiguousarray(np.asarray(mu_w, dtype=np.float64))
    return ii_fhat_factored_kernel(f_arr, rho_idx_c, rho_w_c, mu_idx_c, mu_w_c, fdens_arr)
