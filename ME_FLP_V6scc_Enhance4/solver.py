...
from .jit_kernels import collect_tied_F, fused_argmax_tie, warmup as jit_warmup
    ...
    for out_loop in range(1, config.out_loop_max + 1):
        ...
        for in_loop in range(1, config.in_loop_max + 1):
            ...
            for st in range(config.n_squig, n_stend):
                ...
                # Phase 3: fused @njit kernel computes per-row argmax + max + tie_count
                # in two streaming passes per row, never materializing the full
                # (n_mu, n_delta * n_mup) flat tensor. F-order ravels of om_st and
                # urhs reproduce MATLAB column-major semantics. Bit-exactness vs
                # the Phase 2 NumPy expression is enforced by
                # `parity_enhance2_vs_enhance3.py` at strict tolerance 0.
                om_st_f = np.ascontiguousarray(om_st.reshape(-1, order="F"))
                urhs_f = np.ascontiguousarray(urhs.reshape(-1, order="F"))
                    ...
                    if tied_i_mus.size > 0:
                        ...
                        for i_mu_int in tied_i_mus:
                            ...
                            tied_F_flat = collect_tied_F(
                                mu_val, om_st_f, urhs_f, invalid_F_mask,
                                is_zero_row, float(max_per_mu[i_mu]),
                                int(tie_count[i_mu]),
                            )
